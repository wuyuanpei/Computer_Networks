 ...
 *
 * This class implements a basic packet forwarder for a simplified overlay IP
 * router. It runs as a separate thread.
 *
 * An application layer thread provides new packet payloads to be sent using the
 * provided send() method, and retrieves newly arrived payloads with the
 * receive() method. Each application layer payload is sent as a separate
 * packet, where each packet includes a protocol field, a ttl, a source address
...
public class Forwarder implements Runnable {
	 ...
	 * 
	 * @param myIp  is this node's IP address in the overlay network, expressed as a
	 *              raw integer.
	 * @param sub   is a reference to the Substrate object that this object uses to
	...
	Forwarder(int myIp, Substrate sub, int debug) {
		...
		fromRtr = new ArrayBlockingQueue<Pair<Packet, Integer>>(1000, true);
		toRtr = new ArrayBlockingQueue<Pair<Packet, Integer>>(1000, true);
		quit = false;
	}
				...
				if (pkt.destAdr == myIp) {
					if (pkt.protocol == 1) {
						try {
							...
							System.err.println("Forwarder:run: cannot write" + " to toSnk " + e);
							System.exit(1);
						}
						...
						try {
							...
							System.err.println("Forwarder:run: cannot write" + " to toRtr " + e);
							System.exit(1);
						}
					}
				}
			...
			// else if we have a packet from the Router to send
				...
				if (sub.ready(pp.right)) {
					...
					try {
						...
						System.err.println("Forwarder:run: fromRtr take exception" +e);
						System.exit(1);
					}
				}
			}
			// else if we have a payload from the SrcSnk to send
				...
				if (lnk != -1 && sub.ready(lnk)) {
					...
					try {
						...
						System.err.println("Forwarder:run: fromSrc take exception" +e);
						System.exit(1);
					}
				}
			}
	 ...
	 *                 prefix, the route is updated to use nuLnk. Otherwise, a route
	 *                 is added.
	 *
	...
	public synchronized void addRoute(Prefix nuPrefix, int nuLnk) {
		...
		Pair<Prefix, Integer> nuRte = new Pair<Prefix, Integer>(nuPrefix, nuLnk);
	...
	public Pair<String, String> receive() {
		...
		return new Pair<String, String>(p.payload, Util.ip2string(p.srcAdr));
	}
	...
	public void sendPkt(Packet p, int lnk) {
		...
		try {
			...
			System.err.println("Forwarder:sendPkt: cannot write" + " to fromRtr " + e);
			System.exit(1);
		}
		// debug for print pkt
		if (debug > 2)
	 ...
	 * 
	 * @return a Pair containing the next packet for the Router, including the
	 *         number of the link on which it arrived
	 */
	public Pair<Packet, Integer> receivePkt() {
		...
		try {
			...
			System.err.println("Forwarder:receivePkt: cannot read" + " from toRtr " + e);
			System.exit(1);
		}
		return pp;
	}
	...
	public void printPkt(Packet p, int lnk, int inout) {
		...
		s += String.format("Pkt from %s to %s through lnk %d\n", Util.ip2string(p.srcAdr), Util.ip2string(p.destAdr),
				lnk);
		s += String.format("%s\n", p.payload);
		System.out.println(s);
	}
